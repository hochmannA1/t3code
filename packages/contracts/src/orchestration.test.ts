...
it.effect("ChatFileAttachment accepts metadata over the former 10MB limit", () =>
  Effect.gen(function* () {
    const result = yield* decodeChatFileAttachment({
      type: "file",
      id: "file-1",
      name: "big.bin",
    ...
    assert.strictEqual(result.sizeBytes, 10 * 1024 * 1024 + 1);
  }),
);
