...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const addComposerImages = async (files: File[]) => {
    ...
    // large attachment is being prepared, and the attachments and errors belong
    ...
    for (const file of files) {
      if (!file.type.startsWith("image/") && file.size > PROVIDER_SEND_TURN_MAX_IMAGE_BYTES) {
        error = `'${file.name}' exceeds the ${formatAttachmentSizeLabel(PROVIDER_SEND_TURN_MAX_IMAGE_BYTES)} attachment limit.`;
        continue;
      }
      if (file.type.startsWith("image/") && !isProviderSendTurnSupportedImageMimeType(file.type)) {
