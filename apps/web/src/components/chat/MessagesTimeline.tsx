...
export const MessagesTimeline = memo(function MessagesTimeline({
  ...
  const presentedRows = useMemo(
    ...
    [rawRows, expandedTurnIds, simplified],
  );
...
function LiveWorkEntryTimelineRow({ row }: { row: Extract<TimelineRow, { kind: "work-live" }> }) {
  const ctx = use(TimelineRowCtx);
  const technicalLabel = liveWorkEntryLabel(row.entry, ctx.workspaceRoot, row.active);
  ...
  return (
    ...
    >
      <LiveActivityRow
        label={label}
        iconName={ctx.simplified ? undefined : workEntryIconName(row.entry)}
        toolIcon={ctx.simplified ? undefined : (row.entry.toolIcon ?? row.entry.toolSource?.icon)}
