      ...
      >
        <div className="relative z-10 h-[4.875rem] px-[var(--sidebar-row-content-inset)] py-[var(--sidebar-content-inset)]">
          <div className="flex h-5 min-w-0 items-center gap-1.5">
            ...
            {props.showProjectTitle && props.project ? (
              <ProjectFavicon project={props.project} className="size-4 shrink-0" />
            ) : null}
            <span className="min-w-0 flex-1 truncate text-xs font-medium text-secondary-label">
              {props.showProjectTitle ? props.projectDisplayName : "Draft"}
            </span>
...
const SidebarThreadRow = memo(function SidebarThreadRow(props: {
  ...
  const detailsTooltip = props.simplified ? (
    <TooltipPopup side="right">
      {props.projectDisplayName ? `${thread.title} · ${props.projectDisplayName}` : thread.title}
    </TooltipPopup>
  ) : (
      ...
      >
        <Tooltip disabled={sortable?.isDragging}>
          ...
          >
            ...
            {title}
            {pinIndicator}
            {props.simplified ? null : terminalStatusIcon}
            {isRegeneratingTitle ? (
        ...
        >
          <div
            className={cn(
              ...
              props.simplified ? "h-[3.75rem]" : "h-[4.875rem]",
            )}
          >
            <div className="flex h-5 min-w-0 items-center gap-1.5">
              {draftIndicator}
...
export default function Sidebar() {
  ...
  const prewarmedThreadRefs = useMemo(
    () =>
      getSidebarThreadIdsToPrewarm(orderedThreads).map((thread) =>
        scopeThreadRef(thread.environmentId, thread.id),
      ),
    [orderedThreads],
  );
  ...
  return (
    <>
      {prewarmedThreadRefs.map((threadRef) => (
        ...
        fixedHeader={
          ...
          <SidebarGroup className="relative z-[1] p-[var(--sidebar-content-inset)] pt-1">
            <SidebarThreadHeader
              ...
              newThreadDisabled={appExperience === "code" && projects.length === 0}
