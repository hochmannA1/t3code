...
export interface ProviderServiceLiveOptions {
   ...
   * agent-tool access gates unobservable from a unit test; this seam lets a
...
const makeProviderService = Effect.fn("makeProviderService")(function* (
   ...
   * what disables T3-hosted agent tools everywhere: every adapter already
  ...
  const agentMcpCapabilities = Effect.fn("ProviderService.agentMcpCapabilities")(
    function* (threadId: ThreadId) {
      ...
      if (Object.keys(settings.projectAgentBrowserAccessOverrides).length > 0) {
        // An unresolved project must not bypass explicit browser overrides.
        const thread = Option.isSome(projectionQuery)
          ? yield* projectionQuery.value.getThreadShellById(threadId)
          : Option.none();
        browserAccess = Option.isSome(thread)
          ? resolveProjectAgentBrowserAccess(settings, thread.value.projectId)
          : false;
      }
      if (browserAccess) capabilities.add("preview");
      return capabilities;
    },
    Effect.catch((cause) =>
      Effect.logWarning(
        "Could not read server settings; withholding T3 Code agent tools for this session.",
        { cause },
