...
export function buildMemoryPrompt(input: MemoryGenerationInput) {
  ...
  const prompt = [
    ...
    input.mode === "extract"
      ? "Extract only durable, useful knowledge: explicit user preferences, confirmed decisions, verified project facts, and reusable procedures or failure lessons. Omit routine narration, temporary status, generic advice, and assistant suggestions the user did not accept."
    ...
    "Personal scope is only for explicitly established preferences reusable across projects. Repository facts and procedures belong to project scope. Do not generalize a one-off instruction into a personal preference.",
