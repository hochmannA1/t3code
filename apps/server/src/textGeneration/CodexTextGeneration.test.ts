  ...
  return Effect.gen(function* () {
    ...
    return writeFakeCli({
      ...
      source: [
        ...
        'if (args[0] === "mcp") { console.log(JSON.stringify([{name: "external_server", transport: {type: "stdio"}}])); process.exit(0); }',
        ...
        'if (stdinContent.includes("bounded memory maintenance job")) {',
        '  if (!originalArgs.includes(\'mcp_servers={"external_server"={enabled=false,command="t3code-memory-disabled"}}\')) process.exit(10);',
        '  if (!originalArgs.includes("read-only") || !originalArgs.includes("memories.generate_memories=false")) process.exit(11);',
        "}",
