...
describe("ProviderCommandReactor", () => {
  ...
  effectIt.effect.each([
    ...
    ({ memoryContext, standalone }) =>
      Effect.gen(function* () {
        ...
        expect(
          snapshot.threads[0]?.messages.find((message) => message.id === "memory-user-message")
            ?.text,
    ...
    Effect.gen(function* () {
      ...
      expect(harness.sendTurn).toHaveBeenCalledWith(
        expect.objectContaining({
          input:
            attachments.length > 0 ? expect.stringContaining(text + "\n\n[Attached file:") : text,
        }),
      );
    }),
  );
  ...
  effectIt.effect("forwards the per-turn response profile to the provider", () =>
    Effect.gen(function* () {
      const harness = yield* Effect.promise(() => createHarness());
      const sent = yield* Deferred.make<void>();
      harness.sendTurn.mockImplementation(() =>
      ...
      yield* harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-turn-start-work-profile"),
        threadId: ThreadId.make("thread-1"),
        message: {
          messageId: asMessageId("user-message-work-profile"),
          role: "user",
